...
use willow::engine::{
    self, ActionSink, IoFilterStage, PipelineBuilder, StabilityStage, StaticFilterStage,
};
...
fn unique_test_dir(name: &str) -> PathBuf {
    ...
    let ts = SystemTime::now()
        ...
        .unwrap()
        .as_millis();
    let dir = base.join(format!("e2e_{name}_{ts}"));
...
fn e2e_move_with_static_and_io_conditions_and_logs() {
    ...
    let rule = Arc::new(RuntimeRule {
        event: Event::Any,
        conditions: vec![
            willow::condition::ConditionConfig::Extension {
                value: "jpg".into(),
            }
            .into_condition()
            .unwrap(),
            willow::condition::ConditionConfig::SizeGt { value: 1 }
                .into_condition()
                .unwrap(),
        ],
        actions: vec![
            willow::action::ActionConfig::Log {
                message: "processing {filename}".into(),
            }
            .into_action(),
            willow::action::ActionConfig::Move {
    ...
    let log_dump = { logs.lock().unwrap().clone().join("\n") };
    assert!(
        log_dump.contains("processing image.jpg"),
        "missing custom log in: {log_dump}"
    );
    assert!(
        log_dump.contains("moved") && log_dump.contains("image.jpg"),
        "missing move log in: {log_dump}"
    );
